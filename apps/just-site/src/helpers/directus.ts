import type { CustomDirectusTypes, JustSiteBlogEntries, JustSiteProjectData } from "$directus";
...
export function getRoutes(directus: Directus): Promise<Route[]> {
...
export function getProjectData(directus: Directus): Promise<JustSiteProjectData[]> {
    return directus.request(readItems("just_site_project_data")).then(projectData => {
        ...
        return projectData as JustSiteProjectData[];
    });
}
...
export function getBlogPosts(directus: Directus): Promise<JustSiteBlogEntries[]> {
    return directus.request(readItems("just_site_blog_entries")).then(posts => {
        posts.forEach(p => (p.cover = assetUrl(p.cover as string)));
        return posts as JustSiteBlogEntries[];
    });
}

export function getBlogPost(directus: Directus, slug: string): Promise<JustSiteBlogEntries> {
    return directus
        .request(
