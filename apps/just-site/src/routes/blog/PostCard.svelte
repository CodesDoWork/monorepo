<script lang="ts">
    ...
    import type { JustSiteBlogEntries } from "$directus";
    import Label from "../../components/Label.svelte";

    export let style = "";
    export let post: JustSiteBlogEntries;

    const cover = post.cover as string;
    const dateCreated = post.date_created as string;
</script>
...
<Card class="p-1" style={style}>
    <Link
        ...
        <div class="p-2 pr-2 sm:pr-4 lg:pr-2 xl:pr-4 grid grid-cols-[auto_min-content]">
            ...
            <span class="text-slate-400 dark:text-slate-300">{new Date(dateCreated).toLocaleDateString()}</span>
        </div>
    </Link>
</Card>
