...
export interface DirectusCodegenConfigOptions {
    clientSchema: Types.Schema;
    systemSchema: Types.Schema;
}

export function getCodegenConfig(
    generate?: (options: DirectusCodegenConfigOptions) => Partial<CodegenConfig>,
): CodegenConfig {
    const config = generate ? generate({ clientSchema, systemSchema }) : {};
